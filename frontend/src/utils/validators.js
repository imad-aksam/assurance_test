...
export function validateDriver(data) {
  ...
  if (!data.datePermis) {
    ...
    if (data.dateNaissance) {
      const naissance = new Date(data.dateNaissance);
      const agePermis =
        permis.getFullYear() - naissance.getFullYear() -
...
export function validateVehicle(data, type) {
  const errors = {};
  ...
  } else if (data.valeurNeuf !== '' && Number(data.valeurVenale) > Number(data.valeurNeuf)) {
