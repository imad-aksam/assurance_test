...
import type { FormErrors, InsuranceType, StepInsuranceData } from '../../types/quote';
import { NavButtons } from '../UI';

interface Props {
  data: StepInsuranceData;
  errors: FormErrors;
  onChange: (data: Partial<StepInsuranceData>) => void;
  onNext: () => void;
  onPrev: () => void;
}
...
const INSURANCE_OPTIONS: {
...
}[] = [
  {
    ...
    features: [
      ...
      'Protection tous risques disponible',
    ...
    icon: (
      <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
        ...
        <rect x="20" y="20" width="8" height="6" rx="1" stroke="currentColor" strokeWidth="1.2" fill="none" />
      </svg>
    ),
  },
  {
...
const StepInsurance: React.FC<Props> = ({ data, errors, onChange, onNext, onPrev }) => {
  return (
      ...
      <div className="insurance-cards">
        {INSURANCE_OPTIONS.map((option) => (
          ...
          >
            ...
            {data.typeAssurance === option.value && (
              <div className="card-selected-badge">Sélectionné</div>
            )}
          </button>
        ))}
      </div>
