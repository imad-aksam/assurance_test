...
import type { FormErrors, StepPersonalData } from '../../types/quote';
import { FormField, NavButtons } from '../UI';

interface Props {
  data: StepPersonalData;
  errors: FormErrors;
  onChange: (data: Partial<StepPersonalData>) => void;
  onNext: () => void;
}

const StepPersonal: React.FC<Props> = ({ data, errors, onChange, onNext }) => {
  ...
  return (
    <div className="step-container">
      ...
      <div className="form-grid">
        ...
        <FormField label="Ville" error={errors.cityId as string} required>
          <div className="select-wrapper">
            <select
              className={`form-select ${errors.cityId ? 'input-error' : ''}`}
              value={data.cityId ?? ''}
              onChange={(e) =>
                onChange({ cityId: e.target.value ? Number(e.target.value) : null })
              }
              disabled={loadingCities}
            >
              <option value="">
                {loadingCities ? 'Chargement…' : 'Sélectionnez votre ville'}
              </option>
              {cities.map((city) => (
